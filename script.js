            ...
            <div class="grid grid-cols-12 gap-px bg-gray-700" style="border: 1px solid #374151;">
                ...
                <div class="col-span-12 bg-white p-0" style="border: 1px solid #374151;">
                    <table class="print-table w-full text-xs">
                        ...
                        <tbody>
                            ${(data.ch || []).map(c => `<tr><td>${c.l}</td><td>${c.c}</td><td>${c.s}</td><td>${(parseFloat(c.s || 0) - parseFloat(c.c || 0)).toFixed(2)}</td><td>${c.n}</td></tr>`).join('')}
                            <tr class="font-bold bg-gray-100"><td>TOTAL:</td><td>${totalCost.toFixed(2)}</td><td>${totalSelling.toFixed(2)}</td><td>${totalProfit.toFixed(2)}</td><td></td></tr>
                        </tbody>
                    </table>
                </div>
    ...
    table.innerHTML = `
        ...
        <tfoot>
             <tr id="total-row" class="bg-gray-100 font-bold">
                ...
                <td id="total-cost" class="table-cell text-right">0.00</td>
                <td id="total-selling" class="table-cell text-right">0.00</td>
                <td id="total-profit" class="table-cell text-right">0.00</td>
...
function addChargeRow(data = {}) {
    ...
    newRow.innerHTML = `
        ...
        <td class="table-cell"><input type="number" class="cost-input input-field" value="${data.c || ''}"></td>
        <td class="table-cell"><input type="number" class="selling-input input-field" value="${data.s || ''}"></td>
        <td class="table-cell profit-output bg-gray-50 text-right">${((data.s || 0) - (data.c || 0)).toFixed(2)}</td>
            ...
            filesHtml += `
                <div class="job-file-item border p-3 rounded-lg flex flex-col sm:flex-row justify-between items-center bg-gray-50 hover:bg-gray-100 gap-2">
                    ...
                    <div class="w-full sm:w-1/5 text-xs text-gray-500">
                        <p>Cost: <span class="font-medium text-red-600">KD ${(docData.totalCost || 0).toFixed(2)}</span></p>
                        <p>Profit: <span class="font-medium text-green-600">KD ${(docData.totalProfit || 0).toFixed(2)}</span></p>
                    </div>
                    <div class="space-x-2 flex-shrink-0">
